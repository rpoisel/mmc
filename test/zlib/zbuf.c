...
#define BUF_SIZE_INPUT 1024
#define BUF_SIZE_OUTPUT 1024
#define FILE_SIZE_INPUT 512
#define LEVEL 9
...
int main(int argc, char *argv[])
{
    ...
    unsigned char bufInput[BUF_SIZE_INPUT], bufOutput[BUF_SIZE_OUTPUT];
    int lCnt = 0;
    int lCntRead = 0;

    /* read input file */
    lInput = fopen("/tmp/ncd/in.dat", "rb");
    lCntRead = fread(bufInput, FILE_SIZE_INPUT, 1, lInput);
    if (lCntRead != 1)
    {
        fprintf(stderr, "Sizes do not match: %d\n", lCntRead);
        return -1;
    }
    fclose(lInput);

    /* compress data */
    lCnt = def(bufInput, bufOutput, FILE_SIZE_INPUT, BUF_SIZE_OUTPUT);

    /* write output file */
    lOutput = fopen("/tmp/ncd/out.dat", "wb");
    lCntRead = fwrite(bufOutput, lCnt, 1, lOutput);
    fclose(lOutput);
